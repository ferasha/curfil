...
LabelImage RandomForestImage::improveHistograms(const RGBDImage& image,
		const LabelImage& labelImage, const bool onGPU,
		bool useDepthImages) const {
	...
	if (treeData.size() != ensemble.size()) {
		throw std::runtime_error(
				(boost::format(
						"tree data size: %d, ensemble size: %d. histograms normalized?")
	...
	if (onGPU) {
		...
		cudaSafeCall(
				cudaMemset(deviceProbabilities.ptr(), 0,
						static_cast<size_t>(deviceProbabilities.size()
								* sizeof(float))));

		{
			utils::Profile profile("classifyImagesGPU");
			for (const boost::shared_ptr<const TreeNodes>& data : treeData) {
				classifyImage(treeData.size(), deviceProbabilities, image,
						numClasses, data, useDepthImages, nodeOffsets2);

				//TODO should be change to parallel for and add lock
				for (size_t treeNr = 0; treeNr < ensemble.size(); treeNr++) {
					if (data->getTreeId() == ensemble[treeNr]->getId()) {
						const boost::shared_ptr<
								RandomTree<PixelInstance, ImageFeatureFunction> >& tree =
								ensemble[treeNr]->getTree();
							...
							//	 	 	CURFIL_INFO("calculated offset: "<<offset<<", CPU offset: "<<offset_CPU);
								...
								for (unsigned int i = 0; i < leafSet.size();
									...
									if (leafSet[i] == offset_CPU) {	//CURFIL_INFO("found id");
										...
										LabelType label = labelImage.getLabel(x,
												y);
